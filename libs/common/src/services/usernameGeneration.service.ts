...
export class UsernameGenerationService implements BaseUsernameGenerationService {
  ...
  async generateForwarded(options: any): Promise<string> {
    ...
    } else if (o.forwardedService === "iCloudHME") {
      if (
        o.forwardediCloudHMEEmail == null ||
        o.forwardediCloudHMEEmail === "" ||
        o.forwardediCloudHMEDomain == null ||
        o.forwardediCloudHMEDomain === ""
      ) {
        return null;
      }
      return this.generateiCloudHMEAlias(
        o.forwardediCloudHMEDomain,
        o.forwardediCloudHMEEmail,
        o.website
      );
  ...
  private async generateiCloudHMEAlias(
    domain: string,
    email: string,
    website: string
  ): Promise<string> {
    ...
    url.searchParams.append("label", website);
    url.searchParams.append(
      "note",
      (website != null ? website + " - " : "") + "Generated by Bitwarden."
    );
    ...
    if (response.status === 401) {
      throw "Invalid iCloud HME Domain / Email.";
    }
    throw "Unknown iCloud HME error occurred.";
  }
}
